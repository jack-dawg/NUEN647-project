...
Nq = 6; % order of 1D quadrature rule
%
stdev = 0.01;
...
for i = 1:length(h_tuple)
    for qi = 1:length(multi_wts)
        str = sprintf(formatSpec,qi-1);
        hold1 = csvread(str,1,1);
%         E(qi) = mean(hold1); % QoI is E_hw time averaged
        E(qi) = hold1(length(hold1)); % QoI is E_hw at last time step
%         save(qi,i) = Psi_i(i,multi_pts(qi,:), n_var, P);
        e_hw(i,1) = multi_wts(qi).*E(qi).*Psi_i(i,multi_pts(qi,:), n_var, P) + e_hw(i,1);
    end
%     toc
%     e_hw(i,1) = multi_wts.*E.*save(:,i) + e_hw(i,1);
